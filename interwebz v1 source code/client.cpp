...
void __stdcall MyMovePacket(int sequence_number, float input_sample_frametime, bool active, bool& SendPacket)
{
	...
	if(pWeapon)
	{
		...
		if(!pWeapon->IsMiscWeapon())
		{
			...
			if(g_CVARS.CvarList[AntiAimX] != 0 && !pUserCmd->buttons & IN_USE)
			{
			...
			if (g_CVARS.CvarList[AntiAimY] == 1 && !pUserCmd->buttons & IN_USE) //-180 regular/alligned
